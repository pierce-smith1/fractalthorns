...
type Line = RecordLoader.Line;
type Record = {
    text: Api.RecordTextResponse,
    entry: Api.RedactableRecordEntry,
}
...
export async function search(options: SearchOptions, regex_string: string): Promise<SearchResult> {
    const records = (await RecordQueries.get_all_text())
        .filter(record => record.entry.solved);
    ...
    let matches = await Promise.all(records.map(async record => {
        ...
        return [record.entry.name, matches_in_record] as [string, Array<SearchMatch>];
    }));
...
export async function search_in_record(record: Record, options: SearchOptions, regex_string: string): Promise<Array<SearchMatch>> { 
    if (options.whole_words) {
        regex_string = `\\b${regex_string}\\b`;
    }

    const record_matches_filters = option_is_satisfied_by(options.in_record, record.entry.title)
        || option_is_satisfied_by(options.in_iteration, record.entry.iteration)
    ...
    for (const [line, i] of record.text.lines.map((line, i) => [line, i] as const)) {
        ...
        while ((regex_match = search_regex.exec(line.text)) !== null) {
            // @ts-ignore
            matches.push({record, line, line_index: i, start_index: regex_match.index, matched_text: regex_match[0]});
        }
    };
