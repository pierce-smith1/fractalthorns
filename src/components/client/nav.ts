...
export function set_domain_items(domain: Domain.Domain) {
    const new_items_promise: Promise<Array<Domain.Item>> = (async () => {
        switch (domain) {
            ...
            case "episodic":
                const {chapters: record_chapters} = await Fetchers.get.full_episodic({});
                return record_chapters.flatMap(chapter => chapter.records.map(record => ({
...
export function execute_search(term: string) {
    ...
    const sketch_results = Fetchers.get.domain_search({term, type: "sketch"});
    ...
    sketch_results.then(({results}) => update_search_items(results));
